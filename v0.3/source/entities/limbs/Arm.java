...
public class Arm implements Limb {
  ...
  public Arm(double strength_factor, String name) {//strength compared to adult Human
    this.name = name;
    sublimbs.put("upper arm", (int) Math.round(50 * strength_factor));
    clothing_layers.put("upper arm", new ArrayList<Clothing>());
    sublimbs.put("lower arm", (int) Math.round(50 * strength_factor));
    clothing_layers.put("lower arm", new ArrayList<Clothing>());
    sublimbs.put("hand", (int) Math.round(25 * strength_factor));
    clothing_layers.put("hand", new ArrayList<Clothing>());
    max_lifepoints.put("upper arm", sublimbs.get("upper arm"));
    max_lifepoints.put("lower arm", sublimbs.get("lower arm"));
    max_lifepoints.put("hand", sublimbs.get("hand"));
  }
  ...
  public void receive_damage(String limb, int damage) {
    ...
    if(sublimbs.get(limb) <= 0) {
      ...
      if (limb == "lower arm") {
        sublimbs.remove("hand");
        max_lifepoints.remove("hand");
      }
      is_functional = false;
    }
