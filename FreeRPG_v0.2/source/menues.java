...
public class menues{
  ...
  StringBuilder[] real_map, col_map, map, draw;
  ...
  public int delete() {
    ...
    if (names.size() == 0) {
      System.out.println(color.red("No savegames detected"));
  ...
  public int game() {
    ...
    map = entities.refresh_map(real_map); 
    
    //set up menu
    int hour = (int) Math.floor(time / 3600), minute = (int) Math.floor(time % 3600), second = (int) Math.floor(time % ((hour * 3600) + minute * 60));
    ...
    for (int i = 0; i <= border[3] - border[2]; i++) {
      draw[i] = new StringBuilder(map[border[2] + i].subSequence(border[0], border[1]));
    }
    draw[y].setCharAt(x, 'P');
    ...
    while(!done) {
      ...
      for (; draw_left < menu.size(); draw_left++) {
        System.out.println(draw[draw_left] + color.white_bg(" ") + menu.get(draw_left));
      }
      for (int i = draw_left; i <= border[3] - border[2]; i++) {
        System.out.println(draw[i] + color.white_bg(" "));
      }
      System.out.println(color.white_bg(form.fill(" ")));
      ...
      switch(choose) {
      case 0://movement
        if (player.pos[3] > 0 && col_map[player.pos[3] - 1].charAt(player.pos[2]) != '0'){ player.pos[3] -= 1;
          dtime++;}
        break;
      case 1:
        if (player.pos[3] < map.length - 1 && col_map[player.pos[3] + 1].charAt(player.pos[2]) != '0') {player.pos[3] += 1;
          dtime++;}
        break; 
      case 2:
        if (player.pos[2] > 0 && col_map[player.pos[3]].charAt(player.pos[2] - 1) != '0'){ player.pos[2] -= 1;
          dtime++;}
        break;
      case 3:
        if (player.pos[2] < map[0].length() - 1 && col_map[player.pos[3]].charAt(player.pos[2] + 1) != '0') {player.pos[2] += 1;
          dtime++;}
        break;
      ...
      if (dtime > 0) {   
        ...
        for (int i = 0; i <= border[3] - border[2]; i++) {
          draw[i] = new StringBuilder(map[border[2] + i].subSequence(border[0], border[1])); //adjust draw to distance
        }
        x = 20; y = 20; 
        if (border[1] == map[0].length() - 1) {//far right
          x = draw[y].length() - 1 - border[1] + player.pos[2];
  ...
  private boolean grab() {
    ...
    if (pickable.size() == 0) {
      messages.add("nothing to grab");
      return false;
    }
    else if(player.in_left_hand != null && player.in_right_hand != null) {
      messages.add("no free hand");
      return false;
    }
      
    int counter = 1;
    for (int i = 0; i < pickable.size(); i++) {//output
  ...
  private int talk() {
    ...
    for (int i = 1; i < entities.entities.size(); i++) {
      ...
      if (entities.entities.get(i).getClass().getSimpleName().equals("Human") && 
        ...
        map[partners.get(partners.size() - 1).get_pos()[1]].setCharAt(partners.get(partners.size() - 1).get_pos()[0], counter++);
      }  
    }
    for (int i = 0; i <= border[3] - border[2]; i++) {
      draw[i] = new StringBuilder(map[border[2] + i].subSequence(border[0], border[1]));
    ...
    for (; draw_left < partners.size(); draw_left++) {
      System.out.println(draw[draw_left] + color.white_bg(" ") + counter + ")" + partners.get(draw_left).get_name());
      counter++;
    }
    for (int i = draw_left; i <= border[3] - border[2]; i++) {
      System.out.println(draw[i] + color.white_bg(" "));
    ...
    choice = util.input_numb(0, options.size() - 1);
    //partner answers
    messages = partners.get(choice).conversation(options.get(choice), player, messages);
  ...
  private boolean attack() {
    ...
    for (int i = 1; i < entities.entities.size(); i++) {
      if (Math.abs(entities.entities.get(i).get_pos()[0] - player.pos[2]) <= 1 && Math.abs(entities.entities.get(i).get_pos()[1] - player.pos[3]) <= 1) {
        map[entities.entities.get(i).get_pos()[1]].setCharAt(entities.entities.get(i).get_pos()[0], target++);
    ...
    for (entity etarget : targets.keySet()) {
      System.out.println(draw[draw_left] + color.white_bg(" ") + target++ + ".)" + etarget.get_name());
      draw_left++;
    }
    ...
    //choose target
    int decision = util.input_letter(target);
    util.clear();
    ...
    if (util.rand.nextInt(100) >= 20) {
      ...
      messages.add("you beat " + person.get_name() + " in the " + sublimb + " with your " + type + " dealing " + attack + " damage");
    ...
    while (!done) {
      ...
      if ( choice == 0 ) {done = true;}
        ...
        if (reference.getClass().getName().contains("clothing")) {
          ...
          case 2:
            if (reference == player.in_left_hand) {
              player.wear(player.in_left_hand);
              player.in_left_hand = null;
            } else {
              player.wear(player.in_right_hand);
              player.in_right_hand = null;
            }
            break;
          }
  ...
  private void looking(int[] border) {
    ...
    //set marker
    draw[marker[3]].setCharAt(marker[2], 'X');
    ...
    while(!done) {
      ...
      //remove old cursor
      draw[marker[3]].setCharAt(marker[2], map[marker[1]].charAt(marker[0]));
      ...
      count = 'a';
      if (map[marker[1]].charAt(marker[0]) != real_map[marker[1]].charAt(marker[0])) {
        for (int i = 0; i < entities.entities.size(); i++) {
          if(entities.entities.get(i).get_pos()[0] == marker[0] && entities.entities.get(i).get_pos()[1] == marker[1]) {
            menu.add(count + ")" + entities.entities.get(i).get_name());
            entity_options.put(String.valueOf(count++), entities.entities.get(i));
